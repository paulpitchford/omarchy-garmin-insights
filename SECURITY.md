...
The QML service does not read predictable display-cache paths through `FileView`. It runs the fixed `cache read` backend command, which applies the private-file checks above and the summary or trend byte cap before returning a bounded envelope. QML applies a five-second deadline, validates the envelope and inner contract, and preserves previous valid in-memory state when a read fails.
